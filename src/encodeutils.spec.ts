...
import {getWidth, getHeight, Extent} from 'ol/extent.js';
...
test('WorldToVectorContextTransform', () => {
  ...
  expect(transform2D(LV03_EXTENT, 0, 4, 2, tr2)).toStrictEqual([0, 2000, 3000, 0]); // result is scaled 1000x
...
test('listTiles', () => {
  ...
  const tiles = listTilesCoveringExtentAtResolution(printExtent, printResolution, tileGrid);
  expect(tiles).toEqual([{
    "coord": [10, 530, 362],
    "extent": [704443.6526761837, 5831228.013819527, 743579.411158194, 5870363.772301537],
  }]);
})
