...
function parsePNGFromDataURL(dataURL: string) {
  return new Promise((resolve, reject) => {
...
function parsePNG(data: string): Promise<png.PNG> {
  return data.startsWith('data:image/png;base64,')
    ? parsePNGFromDataURL(data.substring('data:image/png;base64,'.length))
    : parsePNGFromFile(data);
}

async function checkImages(actual: string, expected: string): Promise<number> {
...
test('encodeMVTLayer with immediate API', async () => {
  ...
  const value = await checkImages(results[0].baseURL, expected);
  ...
  const value2 = await checkImages(results2[0].baseURL, expected2);
