import {create, scale, Transform, translate} from 'ol/transform.js';
import {getHeight, getWidth, getBottomLeft, Extent} from 'ol/extent.js';
 ...
 */
export function createWorldToVectorContextTransform(renderExtent: Extent, width: number, height: number): Transform {
  const tr = create();
  ...
  console.assert(Math.abs(r1 / r2 - 1) < 0.01, `extent and canvas don't have same ratio: ${r1}, ${r2}`);
...
export function listTilesCoveringExtentAtResolution(printExtent: Extent, printResolution: number, tileGrid: TileGrid): CoordExtent[] {
